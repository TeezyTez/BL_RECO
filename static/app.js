...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    renderQuality(payload.quality, payload.engine);
...
function renderFields(fields) {
  ...
  Object.entries(labels).forEach(([key, label]) => {
    ...
    if (key === "containers" || key === "marks_and_nos") card.classList.add("wide");
    card.innerHTML = `<strong>${label}</strong><span>${escapeHtml(value || "未识别")}</span>`;
    fieldGrid.appendChild(card);
  });
}

function renderQuality(stats, engine = "") {
  const percent = Math.round((stats?.score || 0) * 100);
  const engineText = engine === "openai_vision" ? "视觉模型" : "规则";
  quality.textContent = `${percent}% 完整度 · ${engineText}`;
}
  ...
  if (Array.isArray(value)) {
    return value.map((item, index) => {
      ...
      const pieces = [
        item.container_no,
        item.container_type,
        item.seal_no ? `Seal ${item.seal_no}` : "",
        item.packages,
        item.gross_weight,
        item.measurement
      ].filter(Boolean);
